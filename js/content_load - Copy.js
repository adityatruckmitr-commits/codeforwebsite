...
export async function loadContent({
  ...
  //console.log(view);
  // Fill header immediately (fast UI feedback)
  const sidebarTitles = resolveTitlesFromSidebar();
  updateHeader(view, {
    subjectTitle: subjectTitle ?? appState.selectedSubject ?? "",
    bookTitle: bookTitle ?? sidebarTitles.bookTitle ?? "",
    unitTitle: unitTitle ?? sidebarTitles.unitTitle ?? "",
    chapterTitle: chapterTitle ?? sidebarTitles.chapterTitle ?? "",
  });
  
  // Build model
  //const vb = chapterItems.find(x => /^(video|playlist)$/i.test(String(x?.type||"")));
  //console.log("[loadContent] video block raw =", vb);
  //console.log("[loadContent] value[0] =", vb?.value?.[0], "typeof =", typeof vb?.value?.[0]);
  ...
  //console.log("FLOWCHART ITEM:", chapterItems.find(x => String(x?.type||"").toLowerCase()==="flowchart"));
...
export function updateHeader(view, { subjectTitle, bookTitle, unitTitle, chapterTitle }) {
  view.headerLeft.replaceChildren(
    el("div", { attrs: { "data-ui": "content-chapter-title" }, text: chapterTitle || " " })
  );
...
export function makePillGroup({ subjectTitle, bookTitle, unitTitle }) {
  ...
  parts.push(el("span", { attrs: { "data-ui": "pill-sep" }, text: "→" }));
  ...
  return el("div", { attrs: { "data-ui": "pill-group" }, children: parts });
}
...
function renderTypeTabs(view, types, selectedType, onSelected) {
  const list = container("div", { attrs: { "data-ui": "content-type-tabs", role: "tablist" } });
  ...
  for (const type of types) {
    const btn = el("button", {
      attrs: {
        ...
        "data-type": type,
        "aria-selected": type === selectedType ? "true" : "false",
      },
...
function renderVideo(model) {
  ...
  if (mode === "video") {
    ...
    const frame = el("iframe", {
      attrs: {
        ...
        "data-ui": "video-frame",
        loading: "lazy",
        title: first.title || "Chapter Video",
        allow:
  ...
  // If original playlist came from ids-only, use Video 1/2/3 labels.
  // If it came from items with titles, show title when provided (fallback to Video N).
  const hasTitles =
    itemsRaw.length &&
  ...
  listItems.forEach(({ id, title }, idx) => {
    ...
    const labelText = hasTitles && title ? title : `Video ${idx + 1}`;
    const label = el("span", {
      attrs: { "data-ui": "playlist-label" },
      text: labelText,
    });
...
function renderTxt(model) {
  ...
  if (!blocks.length) {
    return container("div", { attrs: { "data-ui": "content-panel" }, children: [el("div", { attrs: { "data-ui": "content-empty" }, text: "Text notes not available." })] });
  }
...
async function renderPdf(model) {
  ...
  if (!pdfPath) {
    return container("div", {
      attrs: { "data-ui": "content-panel" },
      children: [
        el("div", { attrs: { "data-ui": "content-empty" }, text: "PDF not available." })
      ]
    });
  }
  ...
  const panel = container("div", { attrs: { "data-ui": "pdf-panel" } });
  const themeFile = appState.themefile;
  const frame = el("iframe", {
    attrs: {
